...
export const RecyclingCenterModal: React.FC<RecyclingCenterModalProps> = ({
  ...
  const validate = () => {
    ...
    if (!name.trim()) newErrors.name = t("center.modal.nameRequired");
    if (!location.trim()) newErrors.location = t("center.modal.locationRequired");
    if (!phone.trim() || phone.trim() === "+95 9") newErrors.phone = t("center.modal.phoneRequired");
    if (!openingHours.trim()) newErrors.openingHours = t("center.modal.hoursRequired");
    if (selectedMaterials.length === 0) newErrors.materials = t("center.modal.materialsRequired");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrors((prev) => ({ ...prev, form: err.message || t("center.modal.saveFailed") }));
    } finally {
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <DialogTitle className="text-xl font-bold text-slate-900">
                {isEditing ? t("center.modal.edit") : t("center.modal.add")}
              </DialogTitle>
              <DialogDescription className="text-xs text-slate-500 mt-0.5">
                {isEditing ? t("center.modal.editDesc") : t("center.modal.addDesc")}
              </DialogDescription>
            </div>
          </div>
        </DialogHeader>
        ...
        <div className="space-y-3.5 text-xs text-slate-700">
          ...
          <div className="space-y-1">
            <label className="font-semibold text-slate-800 flex items-center gap-1.5">
              ...
              {t("center.modal.name")} <span className="text-rose-500">*</span>
            </label>
            <Input
          ...
          <div className="space-y-1">
            <label className="font-semibold text-slate-800 flex items-center gap-1.5">
              ...
              {t("center.modal.location")} <span className="text-rose-500">*</span>
            </label>
            <Input
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="space-y-1">
              <label className="font-semibold text-slate-800 flex items-center gap-1.5">
                ...
                {t("center.modal.phone")} <span className="text-rose-500">*</span>
              </label>
              <Input
            ...
            <div className="space-y-1">
              <label className="font-semibold text-slate-800 flex items-center gap-1.5">
                ...
                {t("center.modal.hours")} <span className="text-rose-500">*</span>
              </label>
              <Input
          ...
          <div className="space-y-2 pt-1 border-t border-slate-100">
            <div className="flex items-center justify-between">
              <label className="font-semibold text-slate-800 flex items-center gap-1.5">
                ...
                {t("center.modal.materials")} <span className="text-rose-500">*</span>
              </label>
            ...
            <div className="flex gap-2 pt-1">
              <Input
                placeholder={t("center.modal.customMaterial")}
